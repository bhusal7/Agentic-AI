...
st.markdown(
    "This tool will draft a LinkedIn post for you, review it itself, "
    "and iterate until it's publish-ready."
)
...
if generate_clicked:
    if not topic.strip():
        st.warning("No topic given. Please enter a topic.")
    else:
        ...
        st.markdown("---")
        st.write(f"**Total attempts:** {final_state['attempt']}")
        st.write(f"**Approved:** {final_state['is_approved']}")

        if not final_state["is_approved"]:
            st.markdown("**Last reviewer feedback:**")
